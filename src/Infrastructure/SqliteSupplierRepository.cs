...
public class SqliteSupplierRepository : ISupplierRepository
{
    ...
    public async Task AddAsync(Supplier entity)
    {
        ...
        await conn.ExecuteAsync(@"INSERT INTO Suppliers (Id, Name, Address, TaxId, BankAccountNumber)
                                VALUES (@Id, @Name, @Address, @TaxId, @BankAccountNumber);", entity);
    }

    public async Task DeleteAsync(Guid id)
    {
        await using var conn = _factory.CreateConnection();
        await conn.ExecuteAsync("DELETE FROM Suppliers WHERE Id = @id", new { id });
    }
    ...
    {
        await using var conn = _factory.CreateConnection();
        return await conn.QuerySingleOrDefaultAsync<Supplier>("SELECT Id, Name, Address, TaxId, BankAccountNumber FROM Suppliers WHERE Id = @id", new { id });
    }

    public async Task UpdateAsync(Supplier entity)
    {
        await using var conn = _factory.CreateConnection();
        await conn.ExecuteAsync(@"UPDATE Suppliers SET Name=@Name, Address=@Address, TaxId=@TaxId, BankAccountNumber=@BankAccountNumber WHERE Id=@Id", entity);
    }
}
