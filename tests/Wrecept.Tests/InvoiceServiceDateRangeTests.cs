...
public class InvoiceServiceDateRangeTests
{
    ...
    public async Task GetByDateRange_ShouldReturnMatches()
    {
        ...
        await repo.AddAsync(new Invoice { Id = Guid.NewGuid(), SerialNumber = "1", IssueDate = new DateOnly(2024,1,10) });
        await repo.AddAsync(new Invoice { Id = Guid.NewGuid(), SerialNumber = "2", IssueDate = new DateOnly(2024,2,10) });
